...
const navItems = [
  ...
  { id: "inventory", label: "Inventario", icon: Warehouse, count: 5 },
  { id: "prep", label: "Preparación", icon: ChefHat },
  { id: "recipes", label: "Recetas", icon: BookOpen },
  { id: "shopping", label: "Compras", icon: ShoppingBasket, count: 12 },
  { id: "receipts", label: "Facturas", icon: ReceiptText },
];

const inventorySeed = [
  { id: 1, name: "Tomate de cocina", category: "Frutas y verduras", unit: "lb", stock: 2, par: 12, reorder: 4, price: 1.25, supplier: "Mercado Central", icon: "🍅" },
  { id: 2, name: "Harina 00", category: "Secos y granos", unit: "lb", stock: 18, par: 24, reorder: 8, price: 0.92, supplier: "Distribuidora Roma", icon: "🌾" },
  { id: 3, name: "Mozzarella fior di latte", category: "Lácteos", unit: "kg", stock: 2.4, par: 8, reorder: 3, price: 8.5, supplier: "Lácteos San Julián", icon: "🧀" },
  { id: 4, name: "Albahaca fresca", category: "Frutas y verduras", unit: "manojos", stock: 1, par: 8, reorder: 3, price: 1.1, supplier: "Mercado Central", icon: "🌿" },
  { id: 5, name: "Aceite de oliva", category: "Aceites y condimentos", unit: "L", stock: 7.5, par: 10, reorder: 3, price: 10.3, supplier: "Selectos", icon: "🫒" },
  { id: 6, name: "Prosciutto", category: "Carnes y embutidos", unit: "kg", stock: 3.8, par: 5, reorder: 1.5, price: 17.8, supplier: "Embutidos del Valle", icon: "🥩" },
  { id: 7, name: "Champiñón", category: "Frutas y verduras", unit: "lb", stock: 3, par: 5, reorder: 2, price: 2.2, supplier: "Pricesmart", icon: "🍄" },
  { id: 8, name: "Vino tinto de cocina", category: "Bebidas", unit: "botellas", stock: 7, par: 6, reorder: 2, price: 7.9, supplier: "La Cava", icon: "🍷" },
];

const recipes = [
  { name: "Pizza Margherita", category: "Pizza", prep: "8 min", cost: 3.48, price: 11, margin: 68, stock: 18, emoji: "🍕", ingredients: 6 },
  { name: "Pizza Prosciutto", category: "Pizza", prep: "10 min", cost: 5.12, price: 14, margin: 63, stock: 12, emoji: "🍕", ingredients: 7 },
  { name: "Salsa pomodoro", category: "Preparación base", prep: "45 min", cost: 7.8, price: null, margin: null, stock: 4, emoji: "🥫", ingredients: 5 },
  { name: "Masa napolitana", category: "Preparación base", prep: "24 h", cost: 0.68, price: null, margin: null, stock: 42, emoji: "🫓", ingredients: 4 },
];
...
const formatMoney = (value) => new Intl.NumberFormat("es-SV", { style: "currency", currency: "USD" }).format(value);
...
function Header({ page, setOpen, onUpload }) {
  const titles = { dashboard: ["Buenos días, Carlos", "Aquí está el pulso de Forno hoy."], inventory: ["Inventario", "Control de existencias y niveles de reposición."], prep: ["Preparación del día", "Organiza la mise en place antes del servicio."], recipes: ["Recetas", "Costos, ingredientes y rendimiento por receta."], shopping: ["Lista de compras", "Una lista inteligente, ordenada por pasillo."], receipts: ["Facturas", "Digitaliza compras y actualiza existencias con IA."] };
  return (
...
function Dashboard({ inventory, setPage }) {
  ...
  return <>
    <section className="service-banner">
      ...
      <div><span>FORNO · SAN BENITO</span><h2>Something is cooking!</h2><p>Todo listo para convertir inventario en servicio.</p></div>
...
function Inventory({ inventory, setInventory }) {
  ...
  function adjust(id, amount) { setInventory(items => items.map(x => x.id === id ? { ...x, stock: Math.max(0, +(x.stock + amount).toFixed(1)) } : x)); }
...
function ShoppingPage({ inventory }) {
  const initial = inventory.filter(x => x.stock / x.par < .7).map(x => ({ ...x, buy: Math.ceil(x.par - x.stock), checked: false }));
...
function ReceiptsPage({ onUpload }) {
